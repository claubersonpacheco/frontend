    ...
    <div v-if="!isLoading" class="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
      <article class="rounded-md border border-red-200 bg-white/90 p-6 shadow-lg shadow-red-900/5 backdrop-blur">
        ...
        <div class="mt-8 flex flex-wrap gap-3">
          <button
            type="button"
            class="inline-flex items-center justify-center rounded-md bg-red-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-red-600/20 transition hover:bg-red-700 disabled:opacity-70"
          ...
          <RouterLink
            :to="{ name: 'users' }"
            class="inline-flex items-center justify-center rounded-md border border-slate-200 bg-white px-6 py-3 text-sm font-medium text-slate-700 transition hover:border-brand-200 hover:text-brand-700"
          >
            Voltar sem excluir
          </RouterLink>
        </div>
      ...
      <aside class="rounded-md border border-ink-900/10 bg-[linear-gradient(180deg,_rgba(74,12,12,0.96),_rgba(127,29,29,0.96))] p-6 text-white shadow-xl shadow-red-900/15">
