...
async function remove() {
  errorMessage.value = ''
  try {
    await permissionsStore.deletePermission(String(route.params.id))
    await router.push({ name: 'permissions' })
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : 'Erro ao excluir permissao.'
  }
}
</script>
...
<template>
  <section class="space-y-6">
    <header class="rounded-md border border-white/70 bg-white/85 p-6 shadow-xl shadow-slate-900/5 backdrop-blur">
      <h1 class="text-3xl font-semibold tracking-tight text-slate-900">Excluir permissao</h1>
    </header>
    <article class="rounded-md border border-red-200 bg-white/90 p-6 shadow-lg shadow-red-900/5 backdrop-blur">
      ...
      <div class="mt-6 flex gap-2">
        ...
        <RouterLink :to="{ name: 'permissions' }" class="rounded-md border border-slate-200 px-6 py-3 text-sm">Cancelar</RouterLink>
      </div>
    </article>
  </section>
</template>
