...
async function handleSubmit() {
  ...
  try {
    await settingsStore.createSetting(form)
  ...
  <section class="space-y-6">
    ...
    <form class="space-y-6" @submit.prevent="handleSubmit">
      ...
      <div class="flex justify-end">
        <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-brand-600 px-6 py-3 text-sm font-semibold text-white disabled:opacity-70" :disabled="isSubmitting">{{ isSubmitting ? 'Salvando...' : 'Criar setting' }}</button>
      </div>
    </form>
  </section>
</template>
