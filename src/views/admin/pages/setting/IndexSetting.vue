    ...
    <header class="rounded-[2rem] border border-white/70 bg-white/85 p-6 shadow-xl shadow-slate-900/5 backdrop-blur">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex flex-wrap gap-3">
          <button type="button" class="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 shadow-sm transition hover:border-brand-200 hover:text-brand-700" @click="loadSettings">Atualizar lista</button>
          <RouterLink :to="{ name: 'settings-create' }" class="inline-flex items-center justify-center rounded-xl bg-brand-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-brand-600/20 transition hover:bg-brand-700">Novo setting</RouterLink>
        </div>
      </div>
    </header>
    ...
    <section class="rounded-[2rem] border border-white/70 bg-white/90 p-6 shadow-lg shadow-slate-900/5 backdrop-blur">
      <div class="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        ...
        <label class="relative block min-w-72">
          ...
          <input v-model="searchTerm" type="search" placeholder="Pesquisar por nome, moodle url ou stream" class="block w-full rounded-xl border-slate-200 bg-white/90 px-4 py-3 pr-11 text-sm text-slate-900 shadow-sm focus:border-brand-500 focus:ring-brand-500" />
        </label>
      </div>
        ...
        <table class="min-w-full divide-y divide-slate-200">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="item in filteredSettings" :key="item.id" class="text-sm text-slate-700">
              ...
              <td class="px-4 py-4"><div class="flex justify-end gap-2"><RouterLink :to="{ name: 'settings-edit', params: { id: item.id } }" class="rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-brand-200 hover:text-brand-700">Editar</RouterLink><RouterLink :to="{ name: 'settings-delete', params: { id: item.id } }" class="rounded-xl border border-red-200 px-3 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50">Excluir</RouterLink></div></td>
            </tr>
          </tbody>
        </table>
      </div>
