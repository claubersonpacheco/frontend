  ...
  <section class="space-y-6">
    ...
    <article class="rounded-md border border-white/70 bg-white/90 p-6 shadow-lg shadow-slate-900/5 backdrop-blur">
      <div class="grid gap-4 sm:grid-cols-2">
        <input v-model="form.fullname" placeholder="Nome completo" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
        <input v-model="form.shortname" placeholder="Nome curto" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
        ...
        <div class="sm:col-span-2 grid gap-4 rounded-md border border-slate-100 bg-slate-50 p-4 sm:grid-cols-[220px_1fr] sm:items-center">
          <img v-if="imagePreview" :src="imagePreview" alt="Preview da imagem do curso" class="h-32 w-full rounded-md object-cover" />
          <div v-else class="flex h-32 items-center justify-center rounded-md border border-dashed border-slate-200 bg-white text-sm text-slate-400">Imagem do curso</div>
      ...
      <div class="mt-6 flex flex-wrap gap-2">
        <button class="rounded-md bg-brand-600 px-6 py-3 text-sm font-semibold text-white disabled:opacity-70" :disabled="isSubmitting" @click="save(false)">
          {{ isSubmitting ? 'Salvando...' : 'Salvar' }}
        </button>
        <button class="rounded-md border border-brand-200 bg-brand-50 px-6 py-3 text-sm font-semibold text-brand-700 disabled:opacity-70" :disabled="isSubmitting" @click="save(true)">
