  ...
  <section class="space-y-6">
    <header class="rounded-md border border-white/70 bg-white/85 p-6 shadow-xl shadow-slate-900/5 backdrop-blur">
      <div class="flex items-center justify-between gap-3">
        ...
        <RouterLink :to="{ name: 'roles-create' }" class="rounded-md bg-brand-600 px-4 py-3 text-sm font-semibold text-white">Novo role</RouterLink>
      </div>
    </header>
          ...
          <tr v-for="item in rolesStore.items" :key="item.id" class="text-sm text-slate-700">
            ...
            <td class="px-4 py-4">
              <div class="flex justify-end gap-2">
                <RouterLink :to="{ name: 'roles-edit', params: { id: item.id } }" class="rounded-md border border-slate-200 px-3 py-2">Editar</RouterLink>
                <RouterLink :to="{ name: 'roles-delete', params: { id: item.id } }" class="rounded-md border border-red-200 px-3 py-2 text-red-600">Excluir</RouterLink>
              </div>
            </td>
          </tr>
        </tbody>
