  ...
  <section class="space-y-6">
    <header class="rounded-md border border-white/70 bg-white/85 p-6 shadow-xl shadow-slate-900/5 backdrop-blur">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <RouterLink :to="{ name: 'videos-create' }" class="inline-flex items-center justify-center rounded-md bg-brand-600 px-4 py-3 text-sm font-semibold text-white">Novo video</RouterLink>
      </div>
    </header>
    ...
    <article class="rounded-md border border-white/70 bg-white/90 p-6 shadow-lg shadow-slate-900/5 backdrop-blur">
      <div class="mb-4 flex justify-end">
        <button type="button" class="rounded-md border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700" @click="loadVideos">Atualizar</button>
      </div>
            ...
            <tr v-for="video in videosStore.items" :key="video.id" class="text-sm text-slate-700">
              ...
              <td class="px-4 py-4">
                <div class="flex justify-end gap-2">
                  <RouterLink :to="{ name: 'videos-view', params: { id: video.id } }" class="rounded-md border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700">Visualizar</RouterLink>
                  <RouterLink :to="{ name: 'videos-edit', params: { id: video.id } }" class="rounded-md border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700">Editar</RouterLink>
                  <button
                    v-if="canDeleteVideos"
                    type="button"
                    class="rounded-md border border-red-200 px-3 py-2 text-sm font-medium text-red-600 disabled:opacity-60"
