...
<template>
  <main class="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(31,157,85,0.18),_transparent_30%),linear-gradient(180deg,_rgba(248,251,246,0.98),_rgba(236,244,237,0.95))] px-4 py-10 sm:px-6 lg:px-8">
    <section class="mx-auto grid max-w-6xl gap-8 overflow-hidden rounded-md border border-white/70 bg-white/85 p-8 shadow-2xl shadow-brand-900/10 backdrop-blur lg:grid-cols-[1.1fr_0.9fr] lg:p-12">
      <div class="space-y-6">
        <span class="inline-flex w-fit rounded-md bg-brand-50 px-4 py-1 text-sm font-semibold text-brand-700">
          Portal do instituto
        </span>
        ...
        <div class="flex flex-col gap-3 sm:flex-row">
          <RouterLink
            ...
            to="/dashboard"
            class="inline-flex items-center justify-center rounded-md bg-brand-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-brand-600/20 transition hover:bg-brand-700"
          >
            Ir para o dashboard
          </RouterLink>
          ...
          <RouterLink
            to="/cursos"
            class="inline-flex items-center justify-center rounded-md border border-brand-200 bg-white px-6 py-3 text-sm font-semibold text-brand-700 transition hover:border-brand-300"
          >
            Ver cursos
          </RouterLink>
          ...
          <RouterLink
            v-if="!isAuthenticated"
            to="/auth/register"
            class="inline-flex items-center justify-center rounded-md border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-brand-200 hover:text-brand-700"
          >
            Criar conta
          </RouterLink>
        </div>
      ...
      <aside class="rounded-md bg-[linear-gradient(180deg,_#143b27,_#1f9d55_62%,_#b9efcb)] p-6 text-white shadow-xl shadow-brand-900/15">
