...
<template>
  <main class="min-h-screen bg-slate-50 px-4 py-10 sm:px-6 lg:px-8">
    <section class="mx-auto grid max-w-5xl overflow-hidden rounded-md border border-white bg-white shadow-xl shadow-slate-900/10 lg:grid-cols-[0.9fr_1.1fr]">
      <aside class="bg-brand-700 p-8 text-white">
        <img v-if="course?.imageUrl" :src="course.imageUrl" :alt="course.fullname" class="mb-6 h-48 w-full rounded-md object-cover" />
        ...
        <p v-if="course" class="mt-6 rounded-md bg-white/10 px-4 py-3 text-sm">
      ...
      <div class="p-8">
        <div class="sm:col-span-2">
          ...
          <div class="mt-5 grid grid-cols-2 rounded-md bg-slate-100 p-1">
            <button
              type="button"
        ...
        <form v-if="activeMode === 'login'" class="mt-6 grid gap-5" @submit.prevent="submitLogin">
          <input v-model="loginForm.identifier" placeholder="Username ou e-mail" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
          <input v-model="loginForm.password" type="password" placeholder="Senha" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
          <div v-if="isPaidCourse()" class="grid gap-4 rounded-md border border-slate-100 bg-slate-50 p-4 sm:grid-cols-2">
            <select v-model="loginForm.paymentMethod" class="rounded-md border-slate-200 px-4 py-3 text-sm">
            ...
            <div v-if="loginForm.paymentMethod === 'pix'" class="rounded-md border border-emerald-100 bg-white px-4 py-3 text-sm font-medium text-emerald-700">PIX a vista</div>
            <select v-else v-model="loginForm.paymentTerm" class="rounded-md border-slate-200 px-4 py-3 text-sm">
            ...
            <input v-if="loginForm.paymentTerm === 'installments'" v-model.number="loginForm.installments" type="number" min="1" :max="course?.maxInstallments ?? 1" placeholder="Parcelas" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
            <div v-if="loginForm.paymentMethod === 'bank_transfer' && course?.bankTransferDetails" class="sm:col-span-2 rounded-md border border-brand-100 bg-white p-4 text-sm leading-6 text-slate-700 whitespace-pre-line">{{ course.bankTransferDetails }}</div>
            <div v-if="loginForm.paymentMethod === 'cash_in_person'" class="sm:col-span-2 rounded-md border border-brand-100 bg-white p-4 text-sm leading-6 text-slate-700">Pagamento em dinheiro no local. O acesso sera liberado apos confirmacao administrativa.</div>
          </div>

          <p v-if="errorMessage" class="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{{ errorMessage }}</p>

          <button class="rounded-md bg-brand-600 px-6 py-3 text-sm font-semibold text-white disabled:opacity-70" :disabled="isSubmitting || !course">
        ...
        <form v-else class="mt-6 grid gap-5 sm:grid-cols-2" @submit.prevent="submitRegister">
          <input v-model="registerForm.username" placeholder="Username" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
          <input v-model="registerForm.name" placeholder="Nome" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
          <input v-model="registerForm.lastname" placeholder="Sobrenome" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
          <input v-model="registerForm.email" type="email" placeholder="E-mail" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
          <input v-model="registerForm.password" type="password" placeholder="Senha" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
          <input v-model="registerForm.confirmPassword" type="password" placeholder="Confirmar senha" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
          <div v-if="isPaidCourse()" class="sm:col-span-2 grid gap-4 rounded-md border border-slate-100 bg-slate-50 p-4 sm:grid-cols-2">
            <select v-model="registerForm.paymentMethod" class="rounded-md border-slate-200 px-4 py-3 text-sm">
              <option value="">Forma de pagamento</option>
              <option v-if="course?.paymentMethods?.includes('pix')" value="pix">PIX</option>
              <option v-if="course?.paymentMethods?.includes('boleto')" value="boleto">Boleto</option>
              <option v-if="course?.paymentMethods?.includes('card')" value="card">Cartao</option>
            ...
            <div v-if="registerForm.paymentMethod === 'pix'" class="rounded-md border border-emerald-100 bg-white px-4 py-3 text-sm font-medium text-emerald-700">PIX a vista</div>
            <select v-else v-model="registerForm.paymentTerm" class="rounded-md border-slate-200 px-4 py-3 text-sm">
              <option v-if="course?.paymentTerms === 'cash' || course?.paymentTerms === 'both'" value="cash">A vista</option>
              <option v-if="course?.paymentTerms === 'installments' || course?.paymentTerms === 'both'" value="installments">A prazo</option>
            </select>
            <input v-if="registerForm.paymentTerm === 'installments'" v-model.number="registerForm.installments" type="number" min="1" :max="course?.maxInstallments ?? 1" placeholder="Parcelas" class="rounded-md border-slate-200 px-4 py-3 text-sm" />
            <div v-if="registerForm.paymentMethod === 'bank_transfer' && course?.bankTransferDetails" class="sm:col-span-2 rounded-md border border-brand-100 bg-white p-4 text-sm leading-6 text-slate-700 whitespace-pre-line">{{ course.bankTransferDetails }}</div>
            <div v-if="registerForm.paymentMethod === 'cash_in_person'" class="sm:col-span-2 rounded-md border border-brand-100 bg-white p-4 text-sm leading-6 text-slate-700">Pagamento em dinheiro no local. O acesso sera liberado apos confirmacao administrativa.</div>
          </div>

          <p v-if="errorMessage" class="sm:col-span-2 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{{ errorMessage }}</p>

          <button class="sm:col-span-2 rounded-md bg-brand-600 px-6 py-3 text-sm font-semibold text-white disabled:opacity-70" :disabled="isSubmitting || !course">
